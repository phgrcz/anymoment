  ...
  render() {
    ...
    return (
      <div>
        <Title
          value={`@${username} ${name}`}
          desc={`${posts} momentos, ${followers} seguidores, seguindo ${followings} - Siga ${username}!`}
        />
          ...
          <Banner />
          {
            !isPrivate(ud.id, isFollowing, ud.account_type) || isAdmin() ?
              ...
              <div style={{ marginTop: 85 }} >
                <Nothing
                  mssg={`O perfil de ${username} é privado.`}
                  secondMssg={`${ mutuals.length != 0 ? humanReadable(mutuals.length, 'seguidores em comum') : '' }`}
                />
              </div>
          }
        </FadeIn>
