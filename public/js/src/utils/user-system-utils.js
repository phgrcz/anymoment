...
export const username_checker = el => {
  ...
  el.on('keyup', async () => {
    ...
    if (value) {
      ...
      if (count == 0) {
        html = '<span class="checker_text">usuário disponível</span><span class="checker_icon"><i class="far fa-smile"></i></span>'
        uc.css({
          width: '160px',
          right: '-188px'
        })
      } else {
        html = '<span class="checker_text">usuário indisponível</span><span class="checker_icon"><i class="far fa-frown"></i></span>'
        uc.css({
          width: '160px',
          right: '-188px'
        })
...
export const quickLogin = options => {
  ...
  $('.q_l_remove').on('click', async e => {
    ...
    Notify({
      value: `${username} removido dos logins recentes`,
