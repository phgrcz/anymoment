...
export default class MembersList extends React.Component {
  ...
  removeMember = async e => {
    ...
    Notify({ value: `${username} removido` })
  }
      ...
      >
            ...
            <span>
              {
                !Me(member)
                  ? mutualUsersCount == 0
                    ? `${firstname} ${surname}` : humanReadable(mutualUsersCount, 'seguidores em comum')
                  : `${firstname} ${surname}`
              }
            </span>
          </div>
        ...
        <div className='m_bottom'>
          <span className='recommend_by' >
            {
              member != added_by ?
                <div>adicionado por <Link to={`/profile/${added_by_username}`} >{ Me(added_by) ? 'você' : added_by_username }</Link></div>
                : null
            }
          </span>

          {
            Me(member) ? <Link to={`/profile/${username}`} className='sec_btn' >Perfil</Link>
              : Me(gd.admin) || isAdmin() ? <a href='#' className='sec_btn' onClick={this.removePrompt} >Remover {isAdmin() ? 'como admin' : null}</a>
                : isFollowing
          ...
          remove ?
            <div>
              ...
              <Prompt
                ...
                content="Este membro será removido do grupo"
                actionText= 'Remover'
