...
export default class FavList extends React.Component {
  ...
  removeFav = async e => {
    ...
    Notify({ value: `${username} removido dos favoritos` })
  }
    ...
    return (
      ...
      >
        ...
        <div className='m_bottom'>
          {
            Me(id) || isAdmin() ?
              <a href='#' className='sec_btn' onClick={this.removeFav} >Remover {isAdmin() ? 'como admin' : ''}</a>
              : null
          }
          {
            Me(user)
              ? <Link to={`/profile/${username}`} className='sec_btn '>Perfil</Link>
              : isFollowing
