...
app.post('/create-group', async (req, res) => {
  ...
  req.checkBody('name', 'Digite um nome').notEmpty()
  req.checkBody('name', 'O nome deve ter menos que 255 caracteres').isLength({ max: 255 })
  ...
  if (!errors.isEmpty()) {
    ...
    if (affectedRows == 1) {
      ...
      createReadStream(`${root}/public/images/anymoment.png`)
      ...
      res.json({ mssg: 'Erro ao criar o grupo' })
    }
...
app.post('/join-group', async (req, res) => {
  ...
  if (!joined) {
    await db.query('INSERT INTO group_members SET ?', member)
    res.json({
      mssg: `${when == 'add_grp_member' ? `${username} Adicionado` : 'Entrou'}`,
      success: true
    })
  } else {
    res.json({
      mssg: `${session == user ? 'você' : username} já está no grupo`
    })
  }
